                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            var user = new Users
                            {
                                ...
                                SequentialBilling = reader.GetInt32(reader.GetOrdinal("users_sequential_billing")),
                                ...
                                EstablishmentName = reader.GetString(reader.GetOrdinal("users_establishment_name")),
                                EstablishmentAddress = reader.GetString(reader.GetOrdinal("users_establishment_address")),
        ...
        public async Task<int> CreateUserAsync(UserViewModel usuario, List<int>? associatedDoctorIds = null)
        {
            using (var connection = new SqlConnection(_dbContext.Database.GetDbConnection().ConnectionString))
            {
                using (var command = new SqlCommand("sp_CreateUser", connection))
                {
                    ...
                    command.Parameters.AddWithValue("@EstablishmentName", usuario.UsersEstablishmentName ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@EstablishmetAddress", usuario.UsersEstablishmentAddress ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@EstablishmentEmissionPoint", usuario.UsersEstablishmentEmissionpoint ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@EstablishmentPointofSale", usuario.UsersEstablishmentPointofsale ?? (object)DBNull.Value);
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            if (await reader.ReadAsync())
                            {
                                ...
                                userDetails = new UserWithDetails
                                {
                                    ...
                                    EstablishmentName = reader.GetString(reader.GetOrdinal("users_establishment_name")),
                                    EstablishmentAddress = reader.GetString(reader.GetOrdinal("users_establishment_address")),
                                    EstablishmentEmissionPoint = reader.GetString(reader.GetOrdinal("users_establishment_emissionpoint")),
                                    EstablishmentPointofsale = reader.GetString(reader.GetOrdinal("users_establishment_pointofsale")),
